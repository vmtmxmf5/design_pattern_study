...
객체의 내부 상태를 외부에 노출하지 않으면서 스냅샷으로 저장하고, 나중에 그 상태로 복원할 수 있게 하는 패턴.

## 언제 사용하는가

- **Undo/Redo 기능이 필요할 때**: 텍스트 에디터, 그래픽 도구 등에서 이전 상태로 되돌리기
- **트랜잭션 롤백이 필요할 때**: 작업이 실패하면 이전 상태로 복구해야 하는 경우
- **객체 캡슐화를 유지하면서 상태를 저장해야 할 때**: 내부 필드를 public으로 열지 않고도 스냅샷을 만들고 싶은 경우
...
**Originator** — 상태를 가진 원본 객체. 자신의 상태를 스냅샷으로 만들고, 스냅샷에서 복원할 수 있다.

**Memento** — 상태 스냅샷을 담는 불변 객체. Originator만 내용에 접근할 수 있다.

**Caretaker** — Memento를 보관하는 관리자. 히스토리 스택을 관리하지만, Memento 내부를 열어보지 않는다.
...
→ `src/examples/memento_example.py` (작성 예정)
